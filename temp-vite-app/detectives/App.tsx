...
function LevelFiveMovements({busy,onComplete}:{busy:boolean;onComplete:()=>Promise<void>}){
  const [destination,setDestination]=useState('');const [destinationReady,setDestinationReady]=useState(false);const [suspect,setSuspect]=useState('');const [feedback,setFeedback]=useState('');
  ...
  return <section className="movement-register" aria-labelledby="movement-title"><p className="eyebrow">REGISTRO DIGITAL · MOVIMIENTOS DEL TALLER</p><h2 id="movement-title">Cruzar el código recuperado</h2><div className="movement-table-wrap"><table><thead><tr><th>Registro</th><th>Operador</th><th>Material</th><th>Destino</th></tr></thead><tbody>{rows.map(row=><tr key={row[0]}>{row.map(cell=><td key={cell}>{cell}</td>)}</tr>)}</tbody></table></div><p>Una fila relaciona cuatro datos. Recuperá primero el código oculto del informe y localizalo en la tabla.</p>{!destinationReady?<div className="movement-check"><label htmlFor="movement-destination">¿A qué destino fue enviado el registro recuperado?</label><input id="movement-destination" value={destination} onChange={event=>setDestination(event.target.value)} placeholder="Destino"/><button type="button" className="unlock-button" onClick={verifyDestination}>VERIFICAR DESTINO</button></div>:<div className="movement-check"><b>¿Qué sospechosa viste trabajando con Resina RX-4?</b><div className="micro-options">{['Vera','Martina','Bruno','León'].map(name=><button type="button" className={suspect===name.toLowerCase()?'selected':''} onClick={()=>setSuspect(name.toLowerCase())} key={name}>{name}</button>)}</div><button type="button" className="unlock-button" disabled={busy||!suspect} onClick={verifySuspect}>REGISTRAR CONEXIÓN</button></div>}{feedback&&<p className="movement-feedback" role="status">{feedback}</p>}</section>;
}
    ...
    <main className="site-shell" aria-busy={busy}><fieldset className="app-controls" disabled={busy}>
      ...
      {screen === 'game' && <section className="game-page">
        ...
        <div className="investigation-panel">
          {level !== 1 && level !== 2 && level <= 7 && <figure className={`scene-frame ${level === 3?'level-three-weather':level === 4?'level-four-maps':level === 0 ? 'storm-layer' : level === 7 ? 'beam-layer' : 'lamp-layer'}`}><img src={level === 0 ? '/los-archivos-f/images/control-room.jpg' : level===3 ? levelVisuals[2] : level===4&&!unlocked ? '/los-archivos-f/images/nivel-4-sala-planos-v1.png' : unlocked ? successVisuals[level - 1] : levelVisuals[level - 1]} alt={level===4?'Sala de cartografía del museo con un plano incompleto sobre la mesa':'Escena del Museo del Faro vinculada con la investigación'} /><span>{unlocked ? 'EVIDENCIA VISUAL DESBLOQUEADA' : 'REGISTRO VISUAL · ARCHIVO F-01'}</span></figure>}
          ...
          {level >= 2 && level <= 7 && (() => {
            ...
            return <section className="level-card">
              ...
              <details key={`digital-${level}`} className="clue-envelope digital-envelope"><summary><span>◉</span><b>{level===3?'Abrir receptor de señales':level===4?'Abrir registro del plano':'Interceptar archivo digital'}<small>{level===3?'Seis registros recuperados · tocar para examinar':level===4?'Informe del sector de mantenimiento':'Una señal de Federica · tocar para revelar'}</small></b><span>+</span></summary><div className="digital-brief">{level === 3 ? <LightSignal onSolved={setLevelThreeReady}/> : <p><Typewriter text={current.digital}/></p>}</div></details>
