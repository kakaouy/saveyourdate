...
function LevelFourStoryDialog({kind,onContinue}:{kind:'intro'|'success';onContinue:()=>void}){
  ...
  useEffect(()=>{ref.current?.showModal();return()=>ref.current?.close();},[]);
  return <dialog ref={ref} className={`level-three-story-dialog level-four-story-dialog ${success?'is-success':'is-intro'}`} aria-labelledby="level-four-story-title" onCancel={event=>event.preventDefault()}><div className="level-three-story-visual"><FedeArtwork src={success?'/los-archivos-f/images/federica-nivel-4-exito-v1.png':'/los-archivos-f/images/federica-nivel-4-inicio-v1.png'} alt={success?'Federica señala la entrada al corredor oculto':'Federica presenta un plano incompleto en la sala de mapas'} playing={playing} variant={success?'level-4-success':'level-4-intro'}><span className="story-rain" aria-hidden="true"/><span className="story-beam" aria-hidden="true"/>{success&&<span className="corridor-lights" aria-hidden="true"/>}</FedeArtwork></div><div className="level-three-story-copy"><p className="eyebrow">{success?'AGENCIA F · NIVEL DESBLOQUEADO':'AGENCIA F · INICIO DEL NIVEL 4'}</p><h2 id="level-four-story-title">{success?'¡Lo tenemos!':'Falta una parte del plano.'}</h2><p>{success?'La ruta oculta atraviesa 9, 3 y 7. El código abrió el conducto de mantenimiento y reveló un pasaje que no aparece en el plano público. Excelente trabajo: sigamos el corredor.':'Encontramos un plano incompleto. Alguien arrancó justo una parte del sector de mantenimiento. Hay varias reconstrucciones posibles, pero solo una respeta la estructura del edificio. Mirá las conexiones, no solamente la forma del fragmento.'}</p><SimulatedPlayer label={success?'Mensaje final de Fede · Nivel 4':'Bienvenida de Fede · Nivel 4'} onPlaying={setPlaying}/><button className="primary-button" type="button" onClick={onContinue}>{success?'CONTINUAR AL NIVEL 5':'COMENZAR LA MISIÓN'} <span>→</span></button></div></dialog>;
}

const lateStories={
  5:{introTitle:'Un registro quedó oculto.',introText:'La luz roja está ocultando información en el informe. Recuperá el dato escondido y cruzalo con los movimientos registrados en el taller.',successTitle:'R-17 deja un nuevo rastro.',successText:'R-17 termina en la Sala de Banderas. El movimiento figura a nombre de T-04 y transporta Resina RX-4, el mismo material que vimos con Martina. Es una conexión importante, pero todavía no alcanza para acusarla.',background:'/los-archivos-f/images/bg-restoration-workshop.png'},
  6:{introTitle:'Cinco señales. Ningún orden visible.',introText:'Hay cinco señales izadas en el mástil, pero no sabemos qué mensaje forman. Mirá la sala, observá sus pistas y usá la Guía G-02.',successTitle:'Las señales indican una ubicación.',successText:'FAROL no parece una contraseña. Es una ubicación. Las señales estaban indicando la linterna superior del faro. Vamos.',background:'/los-archivos-f/images/lens-workshop.jpg'},
...
function LateLevelStoryDialog({level,kind,onContinue}:{level:5|6|7;kind:'intro'|'success';onContinue:()=>void}){
  ...
  return <dialog ref={ref} className="level-three-story-dialog late-level-story-dialog" aria-labelledby="late-story-title" onCancel={event=>event.preventDefault()}>
    <div className="late-story-visual" style={{backgroundImage:`linear-gradient(#03101955,#03101988),url(${story.background})`}}><div className={`late-fede-character ${playing?'is-speaking':''}`}><img src="/los-archivos-f/images/federica-presentadora-v1.png" alt={`Federica presenta el ${success?'resultado':'objetivo'} del nivel ${level}`}/><span className="late-fede-mouth" aria-hidden="true"/></div><span className="story-light" aria-hidden="true"/><span className="story-dust" aria-hidden="true"/></div>
    <div className="level-three-story-copy"><p className="eyebrow">AGENCIA F · {success?'NIVEL DESBLOQUEADO':`INICIO DEL NIVEL ${level}`}</p><h2 id="late-story-title">{success?story.successTitle:story.introTitle}</h2><p>{success?story.successText:story.introText}</p><SimulatedPlayer label={`${success?'Mensaje final':'Bienvenida'} de Fede · Nivel ${level}`} onPlaying={setPlaying}/><button className="primary-button" type="button" onClick={onContinue}>{success?(level===7?'IR A LA ACUSACIÓN FINAL':`CONTINUAR AL NIVEL ${level+1}`):'COMENZAR LA MISIÓN'} <span>→</span></button></div>
  </dialog>;
}

function LevelFiveMovements({busy,onComplete}:{busy:boolean;onComplete:()=>Promise<void>}){
  const [recordRevealed,setRecordRevealed]=useState(false);const [destination,setDestination]=useState('');const [destinationReady,setDestinationReady]=useState(false);const [suspect,setSuspect]=useState('');const [feedback,setFeedback]=useState('');
  const rows=[['R-15','T-02','Solvente','Archivo'],['R-16','T-07','Barniz','Taller'],['R-17','T-04','Resina RX-4','Sala de Banderas'],['R-18','T-03','Tela protectora','Depósito']];
  ...
  if(!recordRevealed)return <section className="red-report-puzzle" aria-labelledby="red-report-title"><p className="eyebrow">INFORME DE CONSERVACIÓN · INTERFERENCIA DETECTADA</p><h2 id="red-report-title">Hay una anotación debajo de la trama roja.</h2><div className="red-report-sheet"><p>R—1— · CONTROL DE SALIDA</p><div className="red-interference" aria-hidden="true"/><strong>SEGUIR SALIDA R-17</strong></div><p>Probá el elemento de lectura incluido en este nivel para separar la información de la interferencia.</p><button type="button" className="unlock-button" onClick={()=>setRecordRevealed(true)}>ACTIVAR VISOR</button></section>;
  return <section className="movement-register" aria-labelledby="movement-title"><p className="eyebrow">REGISTRO DIGITAL · MOVIMIENTOS DEL TALLER</p><h2 id="movement-title">Buscar R-17 en los movimientos</h2><div className="recovered-record" role="status">DATO RECUPERADO · <b>SEGUIR SALIDA R-17</b></div><div className="movement-table-wrap"><table><thead><tr><th>Registro</th><th>Operador</th><th>Material</th><th>Destino</th></tr></thead><tbody>{rows.map(row=><tr key={row[0]}>{row.map(cell=><td key={cell}>{cell}</td>)}</tr>)}</tbody></table></div>{!destinationReady?<div className="movement-check"><label htmlFor="movement-destination">¿A qué destino fue enviado R-17?</label><input id="movement-destination" value={destination} onChange={event=>setDestination(event.target.value)} placeholder="Destino"/><button type="button" className="unlock-button" onClick={verifyDestination}>VERIFICAR DESTINO</button></div>:<div className="movement-check"><b>R-17 lleva Resina RX-4 y figura a nombre de T-04. ¿Qué sospechosa viste trabajando con ese material?</b><div className="micro-options">{['Vera','Martina','Bruno','León'].map(name=><button type="button" className={suspect===name.toLowerCase()?'selected':''} onClick={()=>setSuspect(name.toLowerCase())} key={name}>{name}</button>)}</div><button type="button" className="unlock-button" disabled={busy||!suspect} onClick={verifySuspect}>REGISTRAR CONEXIÓN</button></div>}{feedback&&<p className="movement-feedback" role="status">{feedback}</p>}</section>;
}
    ...
    <main className="site-shell" aria-busy={busy}><fieldset className="app-controls" disabled={busy}>
      ...
      {screen === 'game' && <section className="game-page">
        ...
        <div className="investigation-panel">
          {level !== 1 && level !== 2 && level <= 7 && <figure className={`scene-frame ${level === 3?'level-three-weather':level === 4?'level-four-maps':level===5?'level-five-red-corridor':level === 0 ? 'storm-layer' : level === 7 ? 'beam-layer' : 'lamp-layer'}`}><img src={level === 0 ? '/los-archivos-f/images/control-room.jpg' : level===3 ? levelVisuals[2] : level===4&&!unlocked ? '/los-archivos-f/images/nivel-4-sala-planos-v1.png' : level===5&&!unlocked?'/los-archivos-f/images/bg-hidden-corridor.png':unlocked ? successVisuals[level - 1] : levelVisuals[level - 1]} alt={level===4?'Sala de cartografía del museo con un plano incompleto sobre la mesa':level===5?'Corredor de mantenimiento iluminado por señales rojas':'Escena del Museo del Faro vinculada con la investigación'} /><span>{unlocked ? 'EVIDENCIA VISUAL DESBLOQUEADA' : 'REGISTRO VISUAL · ARCHIVO F-01'}</span></figure>}
          ...
          {level >= 2 && level <= 7 && (() => {
            ...
            return <section className="level-card">
              ...
              {level === 5 && !unlocked && <div className="level-mission"><span>MISIÓN DEL NIVEL</span><p>Recuperar el registro oculto, localizar su destino y relacionar el material con una ficha anterior.</p></div>}
              {level === 6 && !unlocked && <div className="level-mission"><span>MISIÓN DEL NIVEL</span><p>Descubrir el orden de lectura de las cinco banderas y traducir el mensaje con G-02.</p></div>}
              {level === 7 && !unlocked && <div className="level-mission"><span>MISIÓN DEL NIVEL</span><p>Calibrar el mecanismo LF-04 y obtener la combinación sin abrir todavía el Sobre Negro.</p></div>}
              ...
              {level !== 2 && level !== 3 && level!==4 && level!==5 && <details key={`physical-${level}`} className="clue-envelope physical-envelope"><summary><span>⌕</span><b>Examinar documentos físicos<small>Descubrí qué pruebas necesitás en esta etapa</small></b><span>+</span></summary><div className="physical-brief">{current.evidence.map((item) => <b key={item}>{item}</b>)}</div></details>}
