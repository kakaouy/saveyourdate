...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (localPreview) {
      setAgent('Filo'); setActiveSession(true); setHighestLevel(localPreview === 'briefing' ? 0 : localPreview === 'level4-done' ? 5 : localPreview?.startsWith('level4') ? 4 : localPreview === 'level3-done' ? 4 : localPreview === 'level2-done' ? 3 : localPreview.startsWith('level3') ? 3 : 2);
      if (localPreview === 'library') setScreen('library');
      else if (localPreview === 'briefing') setScreen('briefing');
      else { setScreen('game'); setLevel(localPreview === 'level1' ? 1 : localPreview.startsWith('level4') ? 4 : localPreview.startsWith('level3') ? 3 : 2); }
      return;
    }
  ...
  async function submitLevel(event: FormEvent) {
    ...
    if (await gameAction({action:'unlock',level,answer})) { setMessage(`Desbloqueaste: ${levels[level-1].unlock}.`); setAnswer(''); if(level===2)setLevelTwoSuccess(true);if(level===3)setLevelThreeDialog('success');if(level===4)setLevelFourDialog('success'); }
  }
        ...
        <div className="investigation-panel">
          ...
          {level >= 2 && level <= 7 && (() => {
            ...
            return <section className="level-card">
              ...
              {level === 4 && !unlocked && <div className="level-mission"><span>MISIÓN DEL NIVEL</span><p>Reconstruir el plano, calibrar la lámina y seguir la ruta oculta.</p></div>}
