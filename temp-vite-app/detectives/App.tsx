...
function LevelFiveMovements({busy,onComplete}:{busy:boolean;onComplete:()=>Promise<void>}){
  ...
  return <section className="movement-register" aria-labelledby="movement-title"><p className="eyebrow">DATO RECUPERADO · INFORME DE CONSERVACIÓN</p><h2 id="movement-title">Seguir la salida R-17</h2><div className="recovered-record" role="status"><b>SEGUIR SALIDA R-17</b></div><p>Buscá R-17 en la tabla de movimientos física y cruzá los cuatro datos de esa fila.</p>{!destinationReady?<div className="movement-check"><label htmlFor="movement-destination">¿A qué destino fue enviado R-17?</label><input id="movement-destination" value={destination} onChange={event=>setDestination(event.target.value)} placeholder="Destino"/><button type="button" className="unlock-button" onClick={verifyDestination}>VERIFICAR DESTINO</button></div>:<div className="movement-check"><b>Según la tabla física, R-17 lleva Resina RX-4 y figura a nombre de T-04. ¿Qué sospechosa viste trabajando con ese material?</b><div className="micro-options">{['Vera','Martina','Bruno','León'].map(name=><button type="button" className={suspect===name.toLowerCase()?'selected':''} onClick={()=>setSuspect(name.toLowerCase())} key={name}>{name}</button>)}</div><button type="button" className="unlock-button" disabled={busy||!suspect} onClick={verifySuspect}>REGISTRAR CONEXIÓN</button></div>}{feedback&&<p className="movement-feedback" role="status">{feedback}</p>}</section>;
}
