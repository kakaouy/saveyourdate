...
function FedeArtwork({src,alt,playing,variant,children,staticSrc,frames}:{src:string;alt:string;playing:boolean;variant:string;children?:ReactNode;staticSrc?:string;frames?:string[]}){
  const isSequence=Boolean(staticSrc||frames?.length);
  return <div className={`fede-artwork ${variant}`}>{frames?.length?<div className="fede-artwork-loop fede-artwork-frame-sequence" role="img" aria-label={alt}>{frames.map((frame,index)=><img key={frame} src={frame} alt="" aria-hidden="true" style={{animationDelay:`${index*0.55}s`}}/>)}</div>:staticSrc?<picture className="fede-artwork-loop"><source media="(prefers-reduced-motion: reduce)" srcSet={staticSrc}/><img src={src} alt={alt}/></picture>:<img src={src} alt={alt}/>} {!isSequence&&<><span className={`fede-mouth ${playing?'talking':''}`} aria-hidden="true"/><span className="fede-hair" aria-hidden="true"/></>}{children}</div>;
}
