...
function ServilletaArtwork(){
  return <div className="fede-artwork level-3-intro servilleta-story-frames" role="img" aria-label="Federica sostiene una servilleta en la cafetería frente a la tormenta y el faro">
    {servilletaFrames.map((src,index)=><img className={`servilleta-story-frame servilleta-story-frame-${index}`} src={src} alt="" aria-hidden="true" key={src}/>)}
    <span className="servilleta-interior-lights" aria-hidden="true"/><span className="servilleta-lighthouse-beam" aria-hidden="true"/><span className="servilleta-lighthouse-lamp" aria-hidden="true"/>
  </div>;
}
...
function LevelThreeStoryDialog({kind,onContinue}:{kind:'intro'|'success';onContinue:()=>void}){
  ...
  return <section className={`level-three-story-dialog story-page-view ${success?'is-success':'is-intro'}`} aria-labelledby="level-three-story-title">
    <div className="level-three-story-visual">{success?<FedeArtwork src="/los-archivos-f/images/federica-nivel-3-exito-v1.png" alt="Federica junto al receptor de señales encendido" playing={playing} variant="level-3-success"><span className="story-rain" aria-hidden="true"/><span className="story-beam" aria-hidden="true"/><span className="signal-confirmation" aria-hidden="true"/></FedeArtwork>:<ServilletaArtwork/>}</div>
