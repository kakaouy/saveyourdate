...
function LateLevelStoryDialog({level,kind,onContinue}:{level:5|6|7;kind:'intro'|'success';onContinue:()=>void}){
  ...
  useEffect(()=>{ref.current?.showModal();return()=>ref.current?.close();},[]);
  return <dialog ref={ref} className={`level-three-story-dialog late-level-story-dialog late-level-${level}`} aria-labelledby="late-story-title" onCancel={event=>event.preventDefault()}>
