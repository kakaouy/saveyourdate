...
function LateLevelStoryDialog({level,kind,onContinue}:{level:5|6|7;kind:'intro'|'success';onContinue:()=>void}){
  ...
  return <section className={`level-three-story-dialog late-level-story-dialog late-level-${level} story-page-view`} aria-labelledby="late-story-title">
    <div className={`late-story-visual ${level===5&&!success?'level-five-intro-artwork':''} ${level===6&&!success?'level-six-intro-artwork':''} ${level===7&&!success?'level-seven-intro-artwork':''} ${level===7&&success?'level-seven-final-artwork':''}`} style={{backgroundImage:level===6&&!success?'none':`linear-gradient(#03101955,#03101988),url(${level===5&&!success?story.introArtwork:story.background})`}}>{level===6&&!success?<FedeArtwork src={levelSixIntroFrames[0]} frames={levelSixIntroFrames} frameDuration={0.28} alt="Fede presenta la sala de banderas mientras la tormenta continúa tras las ventanas" playing={playing} variant="level-6-intro"/>:level===7&&!success?<FedeArtwork src={levelSevenIntroFrames[0]} frames={levelSevenIntroFrames} frameDuration={0.28} alt="Fede señala las pistas del tablero, gira el rostro hacia adelante y vuelve suavemente a mirar de costado" playing={playing} variant="level-7-intro"/>:level===7&&success?<FedeArtwork src={levelSevenFinalFrames[0]} frames={levelSevenFinalFrames} frameDuration={0.28} alt="Fede abre el compartimento secreto y celebra el hallazgo del diamante" playing={playing} variant="level-7-success"/>:level===5&&success?<FedeArtwork src="/los-archivos-f/images/federica-nivel-5-final-loop.webp" staticSrc="/los-archivos-f/images/federica-nivel-5-final-frame-1.webp" alt="Federica muestra el frasco de resina RX-4 en el taller" playing={playing} variant="level-5-final"/>:level===5?<picture className="level-five-intro-sequence"><source media="(prefers-reduced-motion: reduce)" srcSet="/los-archivos-f/images/federica-nivel-5-inicio-frame-1.webp"/><img src="/los-archivos-f/images/federica-nivel-5-inicio-loop.webp" alt="Federica presenta la entrada al pasillo oculto"/></picture>:<div className={`late-fede-character ${playing?'is-speaking':''}`}><img src="/los-archivos-f/images/federica-presentadora-v1.png" alt={`Federica presenta el ${success?'resultado':'objetivo'} del nivel ${level}`}/><span className="late-fede-mouth" aria-hidden="true"/></div>}{level===5&&!success&&<span className={`level-five-image-mouth ${playing?'talking':''}`} aria-hidden="true"/>}{level===6&&success&&<span className="story-beam" aria-hidden="true"/>}<span className="story-light" aria-hidden="true"/><span className="story-dust" aria-hidden="true"/></div>
  ...
  return (
    <main className={`site-shell ${storyPageOpen?'story-page-active':''}`} aria-busy={busy}><fieldset className="app-controls" disabled={busy}>
      ...
      {screen === 'library' && <section className="library-page">
        <div className="library-intro"><div className="page-heading"><p className="eyebrow dark">BIENVENIDO, AGENTE {agent.toUpperCase()}</p><h1>La cámara de los expedientes</h1><p><Typewriter text="Un archivo te está esperando. Examiná su portada y abrilo para seguir el rastro. Tu avance queda guardado con el código de tu carpeta."/></p></div>
        <button className="switch-code" onClick={() => {setCode(''); setMessage(''); setShowAccess(true);}}>Ingresar otro código de carpeta</button></div><div className="case-grid">
