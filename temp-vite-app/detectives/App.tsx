...
function LateLevelStoryDialog({level,kind,onContinue,onClose}:{level:5|6|7;kind:'intro'|'success';onContinue:()=>void;onClose:()=>void}){
...
export default function Home() {
  ...
  return (
    <main className="site-shell" aria-busy={busy}><fieldset className="app-controls" disabled={busy}>
      ...
      {levelTwoIntro&&<LevelTwoIntroDialog onClose={()=>setLevelTwoIntro(false)} onContinue={()=>setLevelTwoIntro(false)}/>}
      {levelTwoSuccess&&<LevelTwoSuccessDialog onClose={()=>setLevelTwoSuccess(false)} onContinue={()=>{setLevelTwoSuccess(false);continueInvestigation();}}/>}
      {levelThreeDialog&&<LevelThreeStoryDialog kind={levelThreeDialog} onClose={()=>setLevelThreeDialog(null)} onContinue={()=>{if(levelThreeDialog==='success'){setLevelThreeDialog(null);continueInvestigation();}else setLevelThreeDialog(null);}}/>}
      {levelFourDialog&&<LevelFourStoryDialog kind={levelFourDialog} onClose={()=>setLevelFourDialog(null)} onContinue={()=>{if(levelFourDialog==='success'){setLevelFourDialog(null);continueInvestigation();}else setLevelFourDialog(null);}}/>}
      {lateLevelDialog&&<LateLevelStoryDialog level={lateLevelDialog.level} kind={lateLevelDialog.kind} onClose={()=>setLateLevelDialog(null)} onContinue={()=>{const success=lateLevelDialog.kind==='success';setLateLevelDialog(null);if(success)continueInvestigation();}}/>}
