...
export default function TransmissionPlayer({ audioRef, onEnded, onError, onPlaying }: {
  audioRef: RefObject<HTMLAudioElement | null>; onEnded: () => void; onError: () => void; onPlaying: (playing: boolean) => void;
}) {
  ...
  return <div className="transmission-console">
    <label className="transmission-label" htmlFor="transmission-seek">Escuchá el mensaje completo de Federica</label>
    <audio id="federica-audio" ref={audioRef} preload="metadata" src="/los-archivos-f/audio/federica-bienvenida-v2.wav"
      ...
      onTimeUpdate={e=>setPosition(e.currentTarget.currentTime)} onPlay={()=>state(true)} onPause={()=>state(false)}
    ...
    <div className="brass-player" role="group" aria-label="Reproductor de la transmisión de Federica">
      ...
      <input id="transmission-seek" className="brass-seek" type="range" min="0" max={duration || 1} step="0.1" value={position} disabled={!duration}
      ...
      <button className="brass-options" aria-label="Opciones del mensaje" aria-expanded={expanded} aria-controls="transmission-options" onClick={()=>setExpanded(!expanded)}>⋮</button>
    </div>
    {expanded&&<div className="transmission-options" id="transmission-options"><label>Volumen de voz<input type="range" min="0" max="1" step="0.05" value={volume} onChange={e=>{const value=Number(e.target.value);if(audioRef.current){audioRef.current.volume=value;audioRef.current.muted=false;}setMuted(false);setVolume(value);}}/></label><button onClick={()=>{if(audioRef.current)audioRef.current.currentTime=0;setPosition(0);}}>Volver al inicio</button></div>}
    <small className="transmission-footnote"><span aria-hidden="true">⌕</span> También podés descubrir el mensaje en los cuatro documentos.</small>
  </div>;
}
