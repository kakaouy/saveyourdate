...
export default function MissionMap({hintPanel,level,highestLevel,hintsUsed,elapsedSeconds,saveStatus,visitLevel,onMission,onLibrary}:Props) {
 ...
 return <>
  <div className="mission-toolbar"><div className="mission-toolbar-actions"><button ref={trigger} className={`mission-icon-button ${open?'is-open':''}`} aria-expanded={open} aria-controls="mission-map" aria-haspopup="dialog" aria-label={`Misión. ${done} de 7 niveles resueltos`} onClick={()=>setOpen(!open)}><img src="/los-archivos-f/images/mission-scroll.png" alt=""/><span>MISIÓN</span></button><button className="library-icon-button" aria-label="Biblioteca: abrir catálogo de juegos" onClick={onLibrary}><img src="/los-archivos-f/images/biblioteca-libros.png" alt=""/><span>BIBLIOTECA</span></button><button ref={hintsTrigger} className="toolbar-hint-lens" aria-label="Abrir pistas" aria-haspopup="dialog" aria-expanded={hintsOpen} onClick={()=>setHintsOpen(true)}><img src="/los-archivos-f/images/lupa-pista.png" alt=""/><span>PISTAS</span></button><button className="toolbar-game-time" aria-label={`Tiempo transcurrido ${formatTime(elapsedSeconds)}`} title={`Tiempo transcurrido: ${formatTime(elapsedSeconds)}`}><img src="/los-archivos-f/images/reloj-arena.png" alt=""/><span>TIEMPO</span><b>{formatTime(elapsedSeconds)}</b></button></div></div>
