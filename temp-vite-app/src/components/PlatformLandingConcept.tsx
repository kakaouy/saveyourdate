...
const faqGroups = {
  'Primeros pasos': [
    ['¿Puedo probar la plataforma sin registrarme?', 'Sí. La demostración usa datos ficticios para que recorras invitados, confirmaciones, mesas, Living y otros módulos. Nada de lo que hagas allí se guarda.'],
  ...
  Invitaciones: [
    ['¿La invitación está incluida?', 'Sí. Podés elegir un modelo, personalizar textos, colores, fotos e información, y vincularla con las confirmaciones.'],
    ['¿Puedo modificar la información después?', 'Sí. Mientras está en borrador podés editarla y enviarla a revisión antes de publicarla.'],
    ['¿Mis invitados tienen que instalar una aplicación?', 'No. La invitación y las confirmaciones funcionan desde un enlace que se abre en celular o computadora.'],
  ],
  'Invitados y mesas': [
    ['¿Tengo que cargar cada invitado manualmente?', 'No. Podés agregarlos uno por uno o importar una lista desde CSV o Excel y revisar errores antes de guardar.'],
    ['¿Puedo organizar grupos, círculos y mesas?', 'Sí. La información de cada invitado se conecta con sus acompañantes, círculo social, necesidades y ubicación en el salón.'],
    ['¿Qué ve cada proveedor?', 'Cada proveedor accede únicamente a la información y los entregables necesarios para realizar su tarea.'],
  ],
  Planes: [
    ['¿Cómo funcionan los planes y el pago?', 'El plan para anfitriones se contrata por evento. Para organizadores preparamos una propuesta según la cantidad de eventos. Todo se informa antes de confirmar.'],
    ['¿Check-in y álbum colaborativo están disponibles?', 'Todavía no. Ambos módulos están identificados como “Próximamente” para no confundirlos con funciones activas.'],
  ],
  Privacidad: [
    ['¿Cómo protegen la información del evento?', 'Los eventos reales usan accesos privados y permisos por rol. Cada persona ve únicamente lo que necesita para su tarea.'],
    ['¿Venden o usan los datos de mis invitados para publicidad?', 'No. SaveYourDate no vende datos personales ni los utiliza para publicidad de terceros.'],
  ],
} as const;

type FaqTopic = keyof typeof faqGroups;
...
export default function PlatformLandingConcept() {
  ...
  const [faqTopic, setFaqTopic] = useState<FaqTopic>('Primeros pasos');
    ...
    try {
      const response = await fetch('/api/orders/create', {
        ...
        body: JSON.stringify({
          ...
          eventType: data.get('eventType'), eventScope: data.get('eventScope'),
  ...
  return <main className="platform-concept">
    <header className="concept-nav">
      ...
      <div className="concept-nav-actions">
        ...
        <a className="concept-nav-cta" href="/admin">Crear mi evento</a>
    ...
    <section className="concept-contact" id="contacto"><div><span>CONVERSEMOS</span><h2>Contanos qué estás organizando.</h2><p>Con este contexto podemos orientarte desde el primer mensaje, tanto si preparás tu propio evento como si gestionás varios.</p></div><form onSubmit={submitContact}><div className="concept-contact-pair"><label>Nombre<input name="name" autoComplete="name" required /></label><label>Email<input name="email" type="email" autoComplete="email" required /></label></div><label>¿Qué estás organizando?<select name="eventType" defaultValue="" required><option value="" disabled>Elegí una opción</option><option value="boda">Boda</option><option value="quince">Quince años</option><option value="cumpleanos">Cumpleaños</option><option value="corporativo">Evento corporativo</option><option value="otro">Otro evento</option></select></label><fieldset className="concept-contact-scope"><legend>¿Cómo necesitás usar SaveYourDate?</legend><label><input type="radio" name="eventScope" value="one" required />Un evento propio</label><label><input type="radio" name="eventScope" value="many" required />Varios eventos</label></fieldset><label>Mensaje<textarea name="message" rows={4} required /></label><label className="concept-contact-trap" aria-hidden="true">Empresa<input name="company" tabIndex={-1} autoComplete="off" /></label><button type="submit" disabled={contactStatus === 'sending'}>{contactStatus === 'sending' ? 'Enviando…' : 'Enviar consulta →'}</button>{contactStatus === 'success' && <p className="concept-contact-status is-success" role="status">¡Gracias! Tu consulta fue enviada correctamente.</p>}{contactStatus === 'error' && <p className="concept-contact-status is-error" role="alert">No pudimos enviarla. Revisá tu conexión e intentá nuevamente.</p>}</form></section>
    ...
    {plansOpen && <div className="concept-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setPlansOpen(false); }}><section className="concept-plans-modal" role="dialog" aria-modal="true" aria-labelledby="plans-title"><button className="concept-modal-close" type="button" aria-label="Cerrar planes" onClick={() => setPlansOpen(false)}>×</button><header><span>PLANES</span><h2 id="plans-title">¿Un evento o varios?</h2><p>Elegí el alcance. Las funciones principales están incluidas en las dos modalidades.</p></header><div className="concept-plan-shared"><b>SIEMPRE INCLUIDO</b><span>Invitación y RSVP</span><span>Invitados y grupos</span><span>Mesas y salón</span><span>Entregables</span></div><div className="concept-plan-grid"><article><span>PARA ANFITRIONES</span><h3>Mi evento</h3><p>Todo lo necesario para organizar una celebración propia.</p><div className="concept-plan-price"><b>Precio por evento</b><span>Valor y vigencia confirmados antes de activar</span></div><ul><li>Un evento con todos los módulos disponibles</li><li>Accesos para equipo y proveedores</li></ul><div className="concept-plan-actions"><a href="/admin">Crear mi evento</a><a href="/?demo=panel">Probar demo</a></div></article><article className="concept-plan-pro"><span>PARA PROFESIONALES</span><h3>Varios eventos</h3><p>Una operación repetible para organizadores y equipos.</p><div className="concept-plan-price"><b>Propuesta según volumen</b><span>Alcance y acompañamiento adaptados al uso</span></div><label>Eventos estimados en 3 meses<select value={eventsInQuarter} onChange={(event) => setEventsInQuarter(event.target.value)}><option value="1">1 evento</option><option value="3">2 a 3 eventos</option><option value="6">4 a 6 eventos</option><option value="10">7 a 10 eventos</option><option value="more">Más de 10 eventos</option></select></label><div className="concept-plan-actions"><a href={`mailto:hola@saveyourdate.site?subject=Consulta para varios eventos&body=Quiero gestionar ${eventsInQuarter} eventos en 3 meses.`}>Pedir propuesta</a><a href="/?demo=panel">Probar demo</a></div></article></div><p className="concept-plans-footnote">Sin cargos sorpresa: precio, vigencia y forma de pago se informan antes de confirmar.</p></section></div>}
