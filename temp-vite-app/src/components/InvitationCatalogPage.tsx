...
export default function InvitationCatalogPage() {
  ...
  return <main className="catalog-page" onContextMenu={(event) => event.preventDefault()}>
    ...
    <section className="catalog-heading"><span>CATÁLOGO DE INVITACIONES</span><h1>Una al lado de la otra.<br />Elegí la que más te guste.</h1><p>Deslizá cada teléfono para recorrer el modelo. Tocá una invitación para ver todo lo que incluye, probar sus colores y alternar entre celular y pantalla amplia.</p><div className="catalog-filters" aria-label="Tipo de evento">{([['all','Todos'],['wedding','Boda'],['15years','Quince'],['other','Otros eventos']] as const).map(([id,label]) => <button key={id} className={category === id ? 'active' : ''} onClick={() => setCategory(id)}>{label}</button>)}</div></section>
    <section className="catalog-grid" aria-live="polite">{filtered.map((model) => { const colors = model.palettes || fallbackPalette; return <article className="catalog-card" key={model.id}><button className="catalog-phone" type="button" onClick={() => { setSelected(model); setViewport('phone'); }} aria-label={`Ver detalles de ${model.title}`}><span className="catalog-speaker" /><span className="catalog-phone-screen"><CatalogCardPreview model={model} /></span></button><div className="catalog-card-meta"><div><span>{model.category === 'wedding' ? 'BODA' : model.category === '15years' ? 'QUINCE' : 'OTRO EVENTO'}</span><h2>{model.title}</h2></div><div className="catalog-card-colors" aria-label={`${colors.length} colores`}>{colors.map((option) => <i key={option.id} style={{ background: option.color }} title={option.name} />)}</div></div><button className="catalog-card-open" onClick={() => setSelected(model)}>Ver modelo y colores →</button></article>})}</section>
