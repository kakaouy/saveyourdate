export const applySectionDomOrder = (
  ...
  sectionOrder.forEach((sectionId, index) => {
    ...
    const touchesParallax = previousId === 'parallax' || sectionId === 'parallax';
    if (photoGalleryPair || touchesParallax) return;
    ...
    host.style.setProperty('--modular-ornament-scale-x', `var(--modular-ornament-${ornamentSide}-scale-x, 1)`);
