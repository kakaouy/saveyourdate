...
test('Taller: relacionar documentos antes de identificar la réplica; no confundir peso con respuesta',async()=>{
 ...
 try{
  ...
  assert.throws(()=>applyAction(state,{action:'unlock',level:5,answer:'réplica'}));
  ...
  for(const answer of ['12','molde','resina']){
   assert.throws(()=>applyAction(state,{action:'unlock',level:5,answer}));
   assert.deepEqual(state,before);
  }
  for(const answer of ['réplica','copia','réplica del rubí'])
