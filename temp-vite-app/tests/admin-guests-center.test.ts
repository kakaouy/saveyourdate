...
test('comunicaciones reúne invitaciones, recordatorios, avisos y agradecimientos sin mezclar grupo y círculo', () => {
  ...
  assert.match(source, /Enviá desde el WhatsApp del evento/);
  assert.match(source, /No usamos un número central de Save Your Date/);
  assert.match(source, /WhatsApp propio del evento/);
  assert.match(source, /La conexión se realizará en Meta/);
  assert.match(source, /Primero se envía únicamente a la persona organizadora/);
  assert.match(styles, /\.event-whatsapp-path/);
  assert.match(source, /Abrir siguiente en mi WhatsApp/);
  assert.match(source, /type="datetime-local"/);
  assert.match(source, /automaticConnection\?\.status === "connected"/);
  ...
  assert.match(communicationsApi, /WhatsApp Business propio de este evento/);
  assert.match(communicationsApi, /delivery: 'event-whatsapp-business'/);
