...
async function handler(request: Request) {
  ...
  try {
    ...
    if (body.requestType === 'contact') {
      ...
      const safeContactEmail = escapeHtml(email);
      const safeContactMessage = escapeHtml(message).replace(/\n/g, '<br>');
      await sendEmail({
        to: 'saveyourdate.invite@gmail.com',
        replyTo: email,
        subject: `Nueva consulta desde la web — ${name}`,
        idempotencyKey: `website-contact-${crypto.randomUUID()}`,
        html: emailShell(
          'Nueva consulta desde la web',
          `<p><strong>Nombre:</strong> ${safeContactName}</p>
           <p><strong>Email:</strong> <a href="mailto:${safeContactEmail}">${safeContactEmail}</a></p>
           <p><strong>Mensaje:</strong><br>${safeContactMessage}</p>`
        )
      });
      return json({ ok: true });
    }
